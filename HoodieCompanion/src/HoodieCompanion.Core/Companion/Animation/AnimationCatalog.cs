...
public static class AnimationCatalog
{
    ...
    private static IEnumerable<ClipInfo> Build()
    {
        ...
        yield return new(AnimClip.Stretch, "Rest", "Stretches after staying still.", "Long idle", "Internal drive", false, 1.6, 20, true, 45, "Idle", "Idle");
        ...
        yield return new(AnimClip.Wave, "Cursor", "Friendly hello.", "Left click, greeting, goodbye", "User input", false, 1.1, 25, true, 2, "Raise arm", "Lower arm");
        ...
        yield return new(AnimClip.CatchItem, "Inventory", "Catches the object the user gives it.", "Drop on Hoodie", "OLE drop", false, 0.35, 60, false, 0, "Arms up", "InspectItem", 0.06);
        yield return new(AnimClip.InspectItem, "Inventory", "Looks at the new object.", "After catch", "Inventory add", false, 0.5, 60, false, 0, "Holding item", "PutInBackpack");
        yield return new(AnimClip.PutInBackpack, "Inventory", "Tucks it into its hoodie pocket for safe keeping.", "After inspect", "Inventory stored", false, 0.45, 60, false, 0, "Item to pocket", "Idle (sparkle)");
        yield return new(AnimClip.OpenBackpack, "Inventory", "Opens its pocket to show what it keeps.", "Backpack panel opens", "User opened Backpack", false, 0.5, 55, true, 0, "Hands to pocket", "SearchBackpack");
        yield return new(AnimClip.SearchBackpack, "Inventory", "Rummages in its pocket.", "Backpack open / idle inventory check", "Backpack UI / drive", true, 0, 50, true, 0, "OpenBackpack", "PresentItem / Idle");
        yield return new(AnimClip.PresentItem, "Inventory", "Hands the requested object back to the user.", "User opens a Backpack item", "Shell launch", false, 0.7, 60, false, 0, "Item from pocket", "Idle");
